...
const dateParser_1 = __importDefault(require("../services/dateParser"));
const entityExtractor_1 = __importDefault(require("../utils/entityExtractor"));
const contextManager_1 = __importDefault(require("../utils/contextManager"));
...
function isWinfomiEmail(value) {
    const candidate = value.trim().toLowerCase();
    return /^[^\s@]+@winfomi\.com$/i.test(candidate);
}
function extractPotentialEmail(message, payloadEmail) {
    const trimmedMessage = message.trim();
    if (/^[^\s@]+@[^\s@]+$/.test(trimmedMessage)) {
        return trimmedMessage;
    }
    const extracted = entityExtractor_1.default.extractEmail(message);
    if (extracted) {
...
async function handleEmailVerificationFlow(sessionId, message, payloadEmail, res) {
    if (contextManager_1.default.isEmailVerificationLocked(sessionId)) {
        res.json({
    ...
    const awaitingEmail = contextManager_1.default.isAwaitingEmail(sessionId);
    // If user says 'change email', clear stored email and prompt again
    if (/\b(change|update|edit) email\b/i.test(message)) {
        contextManager_1.default.clearEmployeeProfile(sessionId);
        res.json({
    ...
    if (!candidateEmail && contextManager_1.default.hasUserEmail(sessionId) && !awaitingEmail) {
        candidateEmail = contextManager_1.default.getUserEmail(sessionId) ?? null;
    }
    if (candidateEmail) {
    ...
    if (!awaitingEmail && !candidateEmail) {
        contextManager_1.default.setAwaitingEmail(sessionId);
        res.json({
    ...
    if (!candidateEmail) {
        const attempt = contextManager_1.default.incrementEmailAttempts(sessionId);
        const remaining = MAX_EMAIL_ATTEMPTS - attempt;
        if (attempt >= MAX_EMAIL_ATTEMPTS) {
            contextManager_1.default.setEmailVerificationLocked(sessionId, true);
            res.json({
        ...
        contextManager_1.default.updateContext(sessionId, { awaitingEmail: true });
        return true;
    }
    ...
    try {
        ...
        contextManager_1.default.setEmployeeProfile(sessionId, {
            id: lookup.user.id,
...
function calculateWorkingDays(startDate, endDate, isHalfDay = false) {
    if (!startDate) {
        return null;
    }
    const effectiveEnd = endDate || startDate;
    const total = dateParser_1.default.calculateInclusiveDays(startDate, effectiveEnd, isHalfDay);
    return total > 0 ? total : null;
}
// Helper function to extract WFH details using enhanced extractor
function extractWfhDetails(message) {
    return entityExtractor_1.default.extractWfhDetails(message);
}
// Helper function to process WFH request
async function processWfhRequest(sessionId, details) {
    if (!details.date || !details.reason) {
        throw new Error('Missing required fields');
    }
    const employeeName = contextManager_1.default.getEmployeeName(sessionId) || details.employeeName || DEFAULT_EMPLOYEE_NAME;
    const employeeEmail = contextManager_1.default.getUserEmail(sessionId) || null;
    const employeeId = contextManager_1.default.getEmployeeId(sessionId) || null;
    // Check for overlapping leave or WFH on the same date
    // Check leave overlap
    const overlapCheck = await salesforceService.checkLeaveOverlap(employeeName, details.date, details.date);
...
function extractLeaveDetails(message) {
    // Debug logging for troubleshooting half-day parsing
    console.log('[DEBUG] extractLeaveDetails input:', message);
    const parsedDates = dateParser_1.default.parseDates(message);
    const durationInfo = dateParser_1.default.parseDuration(message);
    ...
    if (startDate && durationInfo.durationDays && (!endDate || durationInfo.hasExplicitDuration)) {
        endDate = durationInfo.isHalfDay
            ? startDate
            : dateParser_1.default.projectEndDate(startDate, durationInfo.durationDays);
    }
...
function calculateInclusiveDays(startDate, endDate) {
    const effectiveEnd = endDate ?? startDate;
    return dateParser_1.default.calculateInclusiveDays(startDate, effectiveEnd);
}
    ...
    if (details.startDate) {
        if (details.isHalfDay) {
            ...
            if (!details.endDate || details.endDate === details.startDate) {
                details.endDate = dateParser_1.default.projectEndDate(details.startDate, details.durationDays);
            }
        }
...
async function processLeaveRequest(sessionId, details) {
    ...
    const overlapCheck = await salesforceService.checkLeaveOverlap(employeeName, details.startDate, endDate);
...
function getRequestedLeaveDays(details) {
    if (typeof details.durationDays === 'number' && details.durationDays > 0) {
        return details.durationDays;
    }
    if (details.startDate) {
        const totalDays = dateParser_1.default.calculateInclusiveDays(details.startDate, details.endDate ?? details.startDate, Boolean(details.isHalfDay));
        return totalDays > 0 ? totalDays : null;
    }
    return null;
}
async function enforceLeaveBalance(sessionId, details, res) {
    const requestedDays = getRequestedLeaveDays(details);
    if (!requestedDays || requestedDays <= 0 || !details.leaveType) {
        return false;
    }
    try {
        const userEmail = contextManager_1.default.getUserEmail(sessionId);
        ...
        if (balance && balance.isAvailable === false) {
            const formatDays = (value) => (Number.isInteger(value) ? `${value}` : value.toFixed(1));
...
async function handleLeaveEditRequest(sessionId, pending, editDetails, message, res) {
...
Please provide corrected dates to continue editing your leave request.`,
    ...
    if (newDetails && newDetails.startDate && newDetails.leaveType) {
        ...
        pendingDetails.employeeName = contextManager_1.default.getEmployeeName(sessionId) || pendingDetails.employeeName || DEFAULT_EMPLOYEE_NAME;
        contextManager_1.default.setPendingConfirmation(sessionId, 'leave', pendingDetails);
...
function handleWfhEditRequest(sessionId, pending, editDetails, message, res) {
    const detailsFromForm = editDetails && typeof editDetails === 'object' ? editDetails : null;
    let newDetails = null;
    if (detailsFromForm) {
        newDetails = {
    ...
    if (newDetails && newDetails.date) {
        newDetails.reason = newDetails.reason || 'Personal';
        newDetails.employeeName = contextManager_1.default.getEmployeeName(sessionId) || newDetails.employeeName || DEFAULT_EMPLOYEE_NAME;
        contextManager_1.default.setPendingConfirmation(sessionId, 'wfh', newDetails);
...
const chatController = async (req, res) => {
    try {
        ...
        if (!shouldSkipHistory) {
            contextManager_1.default.addToHistory(sessionId, message, 'unknown');
        }
        ...
        const storedEmail = contextManager_1.default.getUserEmail(sessionId) || undefined;
        if (!contextManager_1.default.getEmployeeId(sessionId)) {
            const handled = await handleEmailVerificationFlow(sessionId, message, payloadEmail || contextEmail || storedEmail, res);
            if (handled) {
                return;
            }
        }
        // Check if user is responding to confirmation
        const pendingConfirmation = contextManager_1.default.getPendingConfirmation(sessionId);
        const confirmation = normalizedConfirmationAction || entityExtractor_1.default.extractConfirmation(message);
        if (!pendingConfirmation && confirmation) {
            const lastRequest = contextManager_1.default.getLastRequest(sessionId);
            if (confirmation === 'yes') {
                if (lastRequest) {
...
Your manager has been notified and will review your request shortly.`,
        ...
        if (pendingConfirmation) {
            ...
            const isEditAction = normalizedIntentOverride === 'edit_leave' || normalizedIntentOverride === 'edit_wfh' || entityExtractor_1.default.isEditRequest(message);
            ...
            if (confirmation === 'yes' && pending) {
                ...
                contextManager_1.default.clearPendingConfirmation(sessionId);
                if (pending.type === 'leave') {
                    try {
                        ...
                        if (leaveResult.isHoliday) {
                            ...
                            contextManager_1.default.saveLeaveConflict(sessionId, {
...
• Type "details" to see more information about the existing leave`;
                            ...
                            // Success!
                            contextManager_1.default.saveLastRequest(sessionId, {
                                type: 'leave',
...
• "Edit" to make changes`;
        ...
        if ((normalizedIntentOverride === 'edit_leave' || normalizedIntentOverride === 'edit_wfh' || entityExtractor_1.default.isEditRequest(message)) && contextManager_1.default.getLastRequest(sessionId) && !contextManager_1.default.isAwaitingConfirmation(sessionId)) {
            const lastReq = contextManager_1.default.getLastRequest(sessionId);
...
Would you like to create a new ${lastReq.type} request?`;
        ...
        const awaitingLeaveDetails = contextManager_1.default.getAwaitingLeaveDetails(sessionId);
...
Example: "Annual leave for family vacation" or "Sick leave for medical appointment"`;
                ...
                contextManager_1.default.setAwaitingLeaveDetails(sessionId, {
            ...
            if (leaveTypeMatch && awaitingLeaveDetails.clarifiedDate) {
                ...
                if (leaveDetails.startDate && leaveDetails.leaveType) {
                    ...
                    contextManager_1.default.clearAwaitingLeaveDetails(sessionId);
                    // Save to pending confirmation instead of creating immediately
                    leaveDetails.reason = leaveDetails.reason || 'Personal';
                    contextManager_1.default.setPendingConfirmation(sessionId, 'leave', leaveDetails);
...
Tap a button below when you're ready.`;
        ...
        const intentResult = aiService.detectIntent(message);
        ...
        contextManager_1.default.addToHistory(sessionId, message, intent);
        switch (intent) {
            ...
            case 'my_requests': {
                try {
                    const employeeName = contextManager_1.default.getEmployeeName(sessionId) || DEFAULT_EMPLOYEE_NAME;
                    const employeeEmail = contextManager_1.default.getUserEmail ? contextManager_1.default.getUserEmail(sessionId) : null;
                    if (!employeeEmail) {
                        res.json({
                    ...
                    if (leaveRequests.length > 0) {
                        ...
                        for (const leave of leaveRequests) {
                            const statusEmoji = leave.Status__c === 'Approved' ? '✅' :
                    ...
                    if (wfhRequests.length > 0) {
                        ...
                        for (const wfh of wfhRequests) {
                            const statusEmoji = wfh.Status__c === 'Approved' ? '✅' :
            ...
            case 'list_requests': {
                ...
                if (type === 'leave')
                    filtered = filtered.filter(r => r.Leave_Type__c);
                else if (type === 'wfh')
                    filtered = filtered.filter(r => r.Date__c);
                // Filter by time
                if (month || year || week) {
...
• "Sick leave from 15th to 17th"`;
                    ...
                    const sessionEmployeeName = contextManager_1.default.getEmployeeName(sessionId) || leaveDetails.employeeName || DEFAULT_EMPLOYEE_NAME;
                    ...
                    if (!ALLOW_PAST_DATES && dateParser_1.default.isPastDate(startDate)) {
...
Please adjust your new request or update the existing leave first.`;
                    ...
                    const effectiveDuration = pendingDetails.isHalfDay ? 0.5 : (pendingDetails.durationDays ?? dateParser_1.default.calculateInclusiveDays(startDate, endDate, false));
...
Select an option below: [Confirm] [Edit] [Cancel].`;
            ...
            case 'apply_wfh':
                ...
                wfhDetails.employeeName = contextManager_1.default.getEmployeeName(sessionId) || wfhDetails.employeeName || DEFAULT_EMPLOYEE_NAME;
                if (wfhDetails.date && wfhDetails.reason) {
                    ...
                    contextManager_1.default.setPendingConfirmation(sessionId, 'wfh', wfhDetails);
...
• Maintain same productivity as office days`;
            ...
            default:
                ...
                fallbackLeaveDetails.employeeName = contextManager_1.default.getEmployeeName(sessionId) || fallbackLeaveDetails.employeeName || DEFAULT_EMPLOYEE_NAME;
                if (fallbackLeaveDetails.startDate && lowerMessage.includes('leave')) {
                    ...
                    contextManager_1.default.setPendingConfirmation(sessionId, 'leave', fallbackLeaveDetails);
                    response = `📋 **Please confirm your leave request:**
...
Tap a button below when you're ready.`;
                ...
                fallbackWfhDetails.employeeName = contextManager_1.default.getEmployeeName(sessionId) || fallbackWfhDetails.employeeName || DEFAULT_EMPLOYEE_NAME;
                if (fallbackWfhDetails.date && lowerMessage.includes('wfh')) {
                    fallbackWfhDetails.reason = fallbackWfhDetails.reason || 'Personal';
                    contextManager_1.default.setPendingConfirmation(sessionId, 'wfh', fallbackWfhDetails);
                    response = `📋 **Please confirm your WFH request:**
...
Tap a button below when you're ready.`;
                ...
                const conversationHistory = contextManager_1.default.getHistory(sessionId, 3);
