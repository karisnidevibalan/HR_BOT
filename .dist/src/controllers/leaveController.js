...
exports.leaveController = {
    async applyLeave(req, res) {
        try {
            ...
            const holidaysPath = path_1.default.join(__dirname, '../../data/holidays.json');
            let holidaysList = [];
            try {
                const holidaysData = await promises_1.default.readFile(holidaysPath, 'utf-8');
            ...
            const conflictHoliday = holidaysList.find(holiday => leaveDates.includes(holiday.date));
            if (conflictHoliday) {
                return res.status(400).json({
